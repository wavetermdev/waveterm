...
type RendererComponentType = {
  new (props: RendererComponentProps): React.Component<
...
function getRemoteStr(rptr: RemotePtrType): string {
  ...
  let username = isBlank(rptr.ownerid)
    ? null
    : GlobalModel.resolveUserIdToName(rptr.ownerid);
...
@mobxReact.observer
class LineAvatar extends React.Component<
  { line: LineType; cmd: Cmd; onRightClick?: (e: any) => void },
  {}
> {
  render() {
...
@mobxReact.observer
class SmallLineAvatar extends React.Component<
  { line: LineType; cmd: Cmd; onRightClick?: (e: any) => void },
  {}
> {
  render() {
    ...
    return (
      <div
        ...
        className={cn(
          "simple-line-status",
          "status-" + status,
          rtnstate ? "has-rtnstate" : null
        )}
      >
        <span className="linenum">{lineNumStr}</span>
        <i
          title={iconTitle}
          className={cn("fa-sharp fa-solid", icon)}
...
> {
    ...
    fetch(url, { headers: fetchHeaders })
      .then((resp) => {
        if (!resp.ok) {
          throw new Error(
            sprintf(
              "Bad fetch response for /api/rtnstate: %d %s",
              resp.status,
              resp.statusText
            )
          );
      ...
      return (
        <React.Fragment>
          ...
          <div key="meta3" className="meta meta-line3 cmdtext-expanded-wrapper">
            <div className="cmdtext-expanded">
  ...
  handleClick() {
    ...
    if (this.lineRef.current != null) {
      ...
      if (
        sel.anchorNode != null &&
        this.lineRef.current.contains(sel.anchorNode) &&
        !isBlank(selText)
      ) {
        return;
      }
  ...
  getIsHidePrompt(): boolean {
    ...
    if (
      !isBlank(line.renderer) &&
      line.renderer != "terminal" &&
      !isNoneRenderer
    ) {
  ...
  getTerminalRendererHeight(cmd: Cmd): number {
    ...
    let usedRows = screen.getUsedRows(
      lineutil.getRendererContext(line),
      line,
      cmd,
      width
    );
    if (usedRows > 0) {
      height =
        ...
        6 +
        11 +
        termHeightFromRows(usedRows, GlobalModel.termFontSize.get());
    }
    return height;
  }
  ...
  render() {
    let { screen, line, width, staticRender, visible, topBorder, renderMode } =
      this.props;
    ...
    return (
      ...
      >
        ...
        <If condition={!this.isMinimised.get()}>
          ...
          <If
            condition={
              rendererPlugin != null && rendererPlugin.rendererType == "simple"
            }
          >
            <SimpleBlobRenderer
          ...
          <If
            condition={
              rendererPlugin != null && rendererPlugin.rendererType == "full"
            }
          >
            <FullRenderer
              rendererContainer={screen}
          ...
          <If condition={cmd.getRtnState()}>
            ...
            >
              ...
              <If condition={rsdiff != null && rsdiff != ""}>
                ...
                <div className="cmd-rtnstate-sep"></div>
                <div className="cmd-rtnstate-diff">
          ...
          <If
            condition={isSelected && !isFocused && rendererType == "terminal"}
          >
...
> {
  render() {
    ...
    return (
      <div className="line line-invalid">
...
class Prompt extends React.Component<
  { rptr: RemotePtrType; festate: Record<string, string> },
  {}
> {
  render() {
    ...
    if (remote && remote.remotevars) {
      if (
        remote.remotevars["sudo"] ||
        remote.remotevars["bestuser"] == "root"
      ) {
        isRoot = true;
      }
    }
    ...
    let remoteElem = (
      <span
        title={remoteTitle}
        className={cn("term-prompt-remote", remoteColorClass)}
      >
        [{remoteStr}]{" "}
      </span>
    );
    let rootIndicatorElem = (
      <span className="term-prompt-end">{isRoot ? "#" : "$"}</span>
    );
    let branchElem = null;
    let pythonElem = null;
        ...
        <div className="line-content">
          <div className="meta">
            <SmallLineAvatar
              line={line}
              cmd={null}
              onRightClick={this.onAvatarRightClick}
            />
            <div className="ts">{formattedTime}</div>
          </div>
...
> {
  ...
  render() {
    ...
    let termHeight = termHeightFromRows(
      usedRows,
