...
let loggerConfig = {
  ...
  transports: [
    new winston.transports.File({
      filename: path.join(scHome, "prompt-app.log"),
      level: "info",
    }),
  ],
};
...
function calcBounds(clientData) {
  ...
  let size = {
    x: 50,
    y: 50,
    width: pdBounds.width - 150,
    height: pdBounds.height - 150,
  };
  if (
    clientData != null &&
    clientData.winsize != null &&
    clientData.winsize.width > 0
  ) {
...
function getContextMenu(): any {
  ...
  let menuItem = new electron.MenuItem({
    label: "Testing",
    click: () => console.log("click testing!"),
  });
...
function getClientData(willRetry: boolean, retryNum: number) {
  ...
  return fetch(url, { headers: fetchHeaders })
    ...
    .catch((err) => {
      if (willRetry) {
        console.log(
          "error getting client-data from local-server, will retry",
          "(" + retryNum + ")"
        );
        return null;
      }
...
function runLocalServer() {
  ...
  proc.on("exit", (e) => {
    ...
    pReject(
      new Error(
        sprintf("failed to start local server (%s)", getLocalServerPath())
      )
    );
