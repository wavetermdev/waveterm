...
type ElectronApi = {
  ...
  onBracketCmd: (
    callback: (event: any, arg: { relative: number }, mods: KeyModsType) => void
  ) => void;
  onDigitCmd: (
    callback: (event: any, arg: { digit: number }, mods: KeyModsType) => void
  ) => void;
  contextScreen: (
    screenOpts: { screenId: string },
  ...
  onLocalServerStatusChange: (
    callback: (status: boolean, pid: number) => void
  ) => void;
};
...
class Screen {
  ...
  constructor(sdata: ScreenDataType) {
    ...
    this.selectedLine = mobx.observable.box(
      sdata.selectedline == 0 ? null : sdata.selectedline,
      { name: "selectedLine" }
    );
  ...
  getWebShareUrl(): string {
    ...
    return sprintf(
      "https://share.getprompt.dev/share/%s?viewkey=%s",
      this.screenId,
      viewKey
    );
  }
  ...
  refocusLine(
    sdata: ScreenDataType,
    oldFocusType: string,
    oldSelectedLine: number
  ): void {
    ...
    if (
      ...
      (curLineFocus.linenum != null &&
        curLineFocus.linenum != sdata.selectedline)
    ) {
  ...
  setAnchor(anchorLine: number, anchorOffset: number): void {
    let setVal =
      anchorLine == null || anchorLine == 0
        ? "0"
        : sprintf("%d:%d", anchorLine, anchorOffset);
  ...
  isActive(): boolean {
    ...
    return (
      this.sessionId == activeScreen.sessionId &&
      this.screenId == activeScreen.screenId
    );
  }
  ...
  screenSizeCallback(winSize: WindowSize): void {
    ...
    let rows = windowHeightToRows(
      winSize.height,
      GlobalModel.termFontSize.get()
    );
  ...
  loadTerminalRenderer(elem: Element, line: LineType, cmd: Cmd, width: number) {
    ...
    if (
      this.focusType.get() == "cmd" &&
      this.selectedLine.get() == line.linenum
    ) {
      termWrap.giveFocus();
    }
    return;
...
class ScreenLines {
  ...
  updateCmd(cmd: CmdDataType): void {
    if (cmd.remove) {
      throw new Error(
        "cannot remove cmd with updateCmd call [" + cmd.lineid + "]"
      );
    }
    ...
    mobx.action(() => {
      ...
      if (!isBlank(sdata.activescreenid)) {
        ...
        if (screen == null) {
          console.log(
            sprintf(
              "got session update, activescreenid=%s, screen not found",
              sdata.activescreenid
            )
          );
        } else {
...
class InputModel {
  ...
  historyType: mobx.IObservableValue<HistoryTypeStrs> =
    mobx.observable.box("screen");
  historyLoading: mobx.IObservableValue<boolean> = mobx.observable.box(false);
  historyAfterLoadIndex: number = 0;
  historyItems: mobx.IObservableValue<HistoryItem[]> = mobx.observable.box(
    null,
    { name: "history-items", deep: false }
  ...
  historyQueryOpts: OV<HistoryQueryOpts> = mobx.observable.box(
    getDefaultHistoryQueryOpts()
  );
  ...
  hasFocus(): boolean {
    ...
    let historyInputElem = document.querySelector(
      ".cmd-input input.history-input"
    );
  ...
  scrollHistoryItemIntoView(hnum: string): void {
    ...
    let buffer = 15;
    let titleHeight = 24;
    let titleDiv: HTMLElement = document.querySelector(
      ".cmd-history .history-title"
    );
    if (titleDiv != null) {
    ...
    if (elemOffset + elemHeight + buffer > endPos) {
      ...
      historyDiv.scrollTop =
        elemOffset - historyDiv.clientHeight + elemHeight + buffer;
      return;
    }
...
class HistoryViewModel {
  ...
  selectedItems: OMap<string, boolean> = mobx.observable.map(
    {},
    { name: "historyview-selectedItems" }
  );
  ...
  doSelectedDelete(): void {
    ...
    let prtn = GlobalModel.showAlert({
      message:
        "Deleting lines from history also deletes their content from your sessions.",
      confirm: true,
    });
    prtn.then((result) => {
      if (!result) {
...
class BookmarksModel {
  ...
  handleDeleteBookmark(bookmarkId: string): void {
    if (
      this.pendingDelete.get() == null ||
      this.pendingDelete.get() != this.activeBookmark.get()
    ) {
  ...
  setShowNoInputMsg(val: boolean) {
    mobx.action(() => {
      ...
      if (val) {
        ...
        this.showNoInputTimeoutId = setTimeout(
          () => this.setShowNoInputMsg(false),
          2000
        );
      } else {
...
class Model {
  ...
  screenMap: OMap<string, Screen> = mobx.observable.map(
    {},
    { name: "ScreenMap", deep: false }
  );
  ws: WSControl;
  ...
  screenLines: OMap<string, ScreenLines> = mobx.observable.map(
    {},
    { name: "screenLines", deep: false }
  ); // key = "sessionid/screenid" (screenlines)
  ...
  authKey: string;
  isDev: boolean;
  activeMainView: OV<"session" | "history" | "bookmarks" | "webshare"> =
    mobx.observable.box("session", { name: "activeMainView" });
  ...
  screenSettingsModal: OV<{ sessionId: string; screenId: string }> =
    mobx.observable.box(null, { name: "screenSettingsModal" });
  ...
  constructor() {
    ...
    this.ws = new WSControl(
      this.getBaseWsHostPort(),
      this.clientId,
      this.authKey,
      (message: any) => this.runUpdate(message, false)
    );
    this.ws.reconnect();
    ...
    this.termFontSize = mobx.computed(() => {
      ...
      if (
        cdata == null ||
        cdata.feopts == null ||
        cdata.feopts.termfontsize == null
      ) {
    ...
    getApi().onLocalServerStatusChange(
      this.onLocalServerStatusChange.bind(this)
    );
    document.addEventListener("keydown", this.docKeyDownHandler.bind(this));
    document.addEventListener(
      "selectionchange",
      this.docSelectionChangeHandler.bind(this)
    );
  ...
  runUpdate(genUpdate: UpdateMessage, interactive: boolean) {
    mobx.action(() => {
      ...
      if (
        oldContext.sessionid != newContext.sessionid ||
        oldContext.screenid != newContext.screenid
      ) {
        this.inputModel.resetInput();
      } else if (
        remotePtrToString(oldContext.remote) !=
        remotePtrToString(newContext.remote)
      ) {
  ...
  runUpdate_internal(
    ...
    if ("ptydata64" in genUpdate) {
      ...
      if (isBlank(ptyMsg.remoteid)) {
        ...
        this.remotesModalModel.receiveData(
          ptyMsg.remoteid,
          ptyMsg.ptypos,
          ptyData
        );
    ...
    if ("sessions" in update || "activesessionid" in update) {
      ...
      if (
        oldActiveSessionId != newActiveSessionId ||
        oldActiveScreenId != newActiveScreenId
      ) {
        this.activeMainView.set("session");
  ...
  updateScreenLines(slines: ScreenLinesType, load: boolean) {
    mobx.action(() => {
      ...
      if (existingWin == null) {
        if (!load) {
          console.log(
            "cannot update screen-lines that does not exist",
            slines.screenid
          );
          return;
        }
  ...
  _loadScreenLinesAsync(newWin: ScreenLines) {
    ...
    let url = new URL(
      GlobalModel.getBaseHostPort() + "/api/get-screen-lines?" + usp.toString()
    );
    ...
    fetch(url, { headers: fetchHeaders })
      ...
      .catch((err) => {
        this.errorHandler(
          sprintf("getting screen-lines=%s", newWin.screenId),
          err,
          false
        );
      });
  ...
  getRemoteByName(name: string): RemoteType {
    for (let i = 0; i < this.remotes.length; i++) {
      if (
        this.remotes[i].remotecanonicalname == name ||
        this.remotes[i].remotealias == name
      ) {
        return this.remotes[i];
      }
    }
...
class CommandRunner {
  ...
  historyPurgeLines(lines: string[]): Promise<CommandRtnType> {
    let prtn = GlobalModel.submitCommand(
      "history",
      "purge",
      lines,
      { nohist: "1" },
  ...
  openEditRemote(remoteid: string): void {
    GlobalModel.submitCommand(
      ...
      "set",
      null,
      { remote: remoteid, nohist: "1", visual: "1" },
      true
    );
  }
  ...
  return fetch(url, { headers: fetchHeaders })
    .then((resp) => {
      if (!resp.ok) {
        throw new Error(
          sprintf(
            "Bad fetch response for /api/ptyout: %d %s",
            resp.status,
            resp.statusText
          )
        );
...
function getPtyData(
  ...
  let url = sprintf(
    GlobalModel.getBaseHostPort() +
      "/api/ptyout?linenum=%d&screenid=%s&lineid=%s",
    lineNum,
    screenId,
    lineId
  );
...
function getRemotePtyData(remoteId: string): Promise<PtyDataType> {
  let url = sprintf(
    GlobalModel.getBaseHostPort() + "/api/remote-pty?remoteid=%s",
    remoteId
  );
  return _getPtyDataFromUrl(url);
}
