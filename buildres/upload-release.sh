...
# Find the DMG file
echo "Finding DMG"
DMG=$(find $BUILDS_DIR -type f -iname "*.dmg")
# Ensure there is only one
NUM_DMGS=$(echo $DMG | wc -l)
if [ "0" -eq "$NUM_DMGS" ]; then
    echo "no DMG found in $BUILDS_DIR"
    exit 1
elif [ "1" -lt "$NUM_DMGS" ]; then
    echo "multiple DMGs found in $BUILDS_DIR"
    exit 1
fi

# Find the Mac zip
echo "Finding Mac zip"
MAC_ZIP=$(find $BUILDS_DIR -type f -iname "*mac*.zip")
# Ensure there is only one
NUM_MAC_ZIPS=$(echo $MAC_ZIP | wc -l)
if [ "0" -eq "$NUM_MAC_ZIPS" ]; then
    echo "no Mac zip found in $BUILDS_DIR"
    exit 1
elif [ "1" -lt "$NUM_MAC_ZIPS" ]; then
    echo "multiple Mac zips found in $BUILDS_DIR"
    exit 1
fi

# Find the Linux zips
echo "Finding Linux zips"
LINUX_ZIPS=$(find $BUILDS_DIR -type f -iname "*linux*.zip")
# Ensure there is at least one
NUM_LINUX_ZIPS=$(echo $LINUX_ZIPS | wc -l)
if [ "0" -eq "$NUM_LINUX_ZIPS" ]; then
    echo "no Linux zips found in $BUILDS_DIR"
    exit 1
fi
...
RELEASES_CONTENTS="{\"name\": \"$UVERSION\", \"notes\": \"\", \"url\": \"https://https://waveterm-test-autoupdate.s3.us-west-2.amazonaws.com/autoupdate/$MAC_ZIP_NAME\"}"
