...
Show information about a file. This includes the name, mode (permissions), mtime (last modified time), and size (file size in bytes).
...
Remove a file. This is a permanent removal that acts much the same way as `rm`. Recovery is only possible by using a method that would work with the `rm` command.
