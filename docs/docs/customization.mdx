...
```bash
# Set a background image with 50% opacity (default)
wsh setbg ~/pictures/background.jpg

# Set a more transparent background
wsh setbg --opacity 0.3 ~/pictures/light-pattern.png

# Create a tiled pattern background
wsh setbg --tile --opacity 0.2 ~/pictures/texture.png
```

You can use any JPEG, PNG, GIF, WebP, or SVG image as your background. To remove a background, clear the background metadata:

```bash
wsh setmeta -b tab "bg:*=true"
```
