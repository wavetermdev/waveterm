...
The `setbg` command allows you to set a background image for the current tab with customizable opacity and tiling options.

```bash
wsh setbg [--opacity value] [--tile] image-path
```

The command accepts a path to an image file and will set it as the background for the current tab. The image can be displayed either as a cover (default) or tiled, and you can adjust its opacity.
...
```bash
# Set a background image with default settings
...
# Set a vector background
wsh setbg ~/pictures/pattern.svg
```

The command will validate that:

- The provided path points to an accessible image file
