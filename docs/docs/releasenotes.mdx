...
### v0.14.5-fork &mdash; Jun 4, 2026

**SSH Port Forwarding:**

- **`LocalForward` and `RemoteForward` support** &mdash; Port forwarding rules from `~/.ssh/config` are now automatically applied when connecting through Wave. Rules can also be defined in `connections.json` via `ssh:localforward` and `ssh:remoteforward` keys.
