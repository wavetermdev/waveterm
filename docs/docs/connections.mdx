...
|ForwardAgent| Specifies whether the connection to the ssh agent will be forwarded to the remote machine. It can accept `yes` or `no`. The default value is `no`.|
