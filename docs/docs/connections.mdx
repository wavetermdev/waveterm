...
- `[user]@[host]` for a desired SSH remote or type

For WSL Connections:

- `wsl://<distribution name>` for a desired WSL distribution. Alternatively, if the connection already exists in the dropdown list, you can either click it or navigate to it with arrow keys and press enter to connect.
...
:::info
AWS S3 connections only work in the directory widget
