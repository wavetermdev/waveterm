...
While Wave will automatically disable `wsh` if you click the option to delete it, there are cases where you may install it, but for some reason, it doesn't work on your remote machine. In this case, you may wish to disable wsh after the fact. While it is possible to get the prompt again by deleting `wsh` from the remote system, it is also possible to do so by editing the config file. Suppose the connection shows up in the dropdown as `root@wshless`. Then:
