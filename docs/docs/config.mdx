...
| preview:openfileinnewblock <VersionBadge version="v0.15" />            | bool     | set to true to open files in a new preview block when double-clicking or pressing Enter in the directory preview, instead of replacing the current block (defaults to false; directories still navigate in place)                                              |
