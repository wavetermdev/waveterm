...
| agent:allowremotelocalcontrol        | bool     | Allow remote `wsh` sessions to control the local machine (defaults to false)                                                                                                                                                                                  |
| agent:allowbrowsercontrol            | bool     | Allow `wsh` to drive the embedded web widget via CDP (`web snapshot`, `web screenshot`, `web run`, `web get`). Defaults to false. Remote-origin sessions also require `agent:allowremotelocalcontrol`                                                         |
