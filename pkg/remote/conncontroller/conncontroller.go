...
func (conn *SSHConn) connectInternal(ctx context.Context) error {
	...
	if err != nil {
		log.Printf("error: failed to connect to client %s: %+#v", conn.GetName(), err)
		return err
	}
	...
	if err != nil {
		log.Printf("error: unable to open domain socket listener for %s: %+#v", conn.GetName(), err)
		return err
	}
	installErr := conn.CheckAndInstallWsh(ctx, clientDisplayName, nil)
	if installErr != nil {
		log.Printf("error: unable to install wsh shell extensions for %s: %+#v", conn.GetName(), err)
		return fmt.Errorf("conncontroller %s wsh install error: %v", conn.GetName(), installErr)
	}
	csErr := conn.StartConnServer()
	if csErr != nil {
		log.Printf("error: unable to start conn server for %s: %+#v", conn.GetName(), csErr)
